...
func NewDataFrame(mem memory.Allocator, schema *arrow.Schema, arrs []array.Interface) (*DataFrame, error) {
	...
	if df.schema == nil {
		return nil, errors.Errorf("dataframe: nil schema")
	}

	if len(df.schema.Fields()) != len(arrs) {
		return nil, errors.Errorf("dataframe: inconsistent schema/arrays")
	}
	...
	for i, arr := range arrs {
		...
		if int64(arr.Len()) < df.rows {
			return nil, errors.Errorf("dataframe: column %q expected length >= %d but got length %d", ft.Name, df.rows, arr.Len())
		}
	}
...
func (df *DataFrame) ApplyToColumn(columnName, newColumnName string, fn ApplyToColumnFunc) (*DataFrame, error) {
	return df.Apply(func(df *DataFrame) (*DataFrame, error) {
		...
		smartBuilder := NewSmartBuilder(builder, schema)
...
func (df *DataFrame) validate() error {
	...
	for i, col := range df.cols {
		if !col.Field().Equal(df.schema.Field(i)) {
			return errors.Errorf("dataframe validate(): column field %q is inconsistent with schema", col.Name())
		}
		colLen := columnLen(col)
		if colLen < df.rows {
			return errors.Errorf("dataframe validate(): column %q expected length >= %d but got length %d", col.Name(), df.rows, colLen)
		}
	}
	return nil
}
